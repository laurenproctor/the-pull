...
function renderStudyPerformance(studyId,study,posts){
 ...
 if(window.PullCharts){
  const chart=window.PullCharts,counts={};sentiment.comments.forEach(c=>{const day=c.publishedAt.slice(0,10);counts[day]=(counts[day]||0)+1});const dates=Object.keys(counts).sort();
  const timeline=dates.length?chart.figure({type:'bar',title:'When the conversation arrived',unit:'comments',dates:true,labels:dates,series:[{name:'Saved third-party comments',values:dates.map(d=>counts[d])}],note:'Publication dates (UTC). Gaps mean unobserved days, not zero conversation.'}):'<div class="pull-chart-empty"><strong>Conversation over time</strong><p>No dated third-party comments have been collected. Add source-backed comments to a post below to begin the chart.</p></div>';
  const breakdown=sentiment.analyzed?chart.figure({type:'bar',title:'What the collected sample says',unit:'%',fixedMax:100,labels:Object.keys(sentiment.labels),series:[{name:'Share of classified comments',values:Object.values(sentiment.labels).map(n=>n/sentiment.analyzed*100)}],note:sentiment.analyzed+' classified comments. Unclassified and brand-authored comments excluded.'}):'<div class="pull-chart-empty"><strong>Audience response</strong><p>Sentiment remains unmeasured until third-party comments are classified. No before/after claim is available.</p></div>';
  target.insertAdjacentHTML('afterbegin',`<section class="data-spread"><p class="section-label">Response, in context</p><h2>Read the pattern. Check the evidence.</h2><p>Charts describe saved research for this study, not the whole conversation or the effect on sales.</p><div class="data-spread-grid"><div><div class="data-big">${sentiment.comments.length}<small> saved third-party comments</small></div>${timeline}</div><div><div class="data-big">${sentiment.analyzed?Math.round(sentiment.labels.positive/sentiment.analyzed*100)+'%':'—'}<small> positive within the classified sample</small></div>${breakdown}</div></div><p class="data-annotation">Collection: dated, source-linked comments saved manually through each post’s collection form. No API is connected. Plan 2–4 analyst hours per report at an assumed $50/hour, excluding provider fees. Timing reveals attention patterns; sentiment describes the response. Neither establishes causation or represents all customers.</p></section>`);
 }
